...
- **Excerpt precision**: the excerpt shown is the chunk's content up to 500 characters — matching the chunk size ([Ingestion Pipeline](ingestion-pipeline.md)), so in practice it's normally the *whole* chunk, not a prefix of it. Tried a 200-char prefix first: verified live that a citation could resolve to a chunk that genuinely supported the claim, while the 200-char preview cut off before reaching the actual cited sentence — not a fabrication, but it looked like one, which undercuts the "checkable" citation story just as much. Showing the full chunk removes that ambiguity entirely.
